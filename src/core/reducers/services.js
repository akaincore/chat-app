...
export const isFetching = (state = initialState.isFetching, action) => {
  switch (action.type) {
    ...
    case types.RECEIVE_AUTH_PENDING: {
      return { ...state, receiveAuth: true };
    }
    case types.FETCH_ALL_CHATS_PENDING: {
      return { ...state, allChats: true };
    }
    case types.FETCH_MY_CHATS_PENDING: {
      return { ...state, myChats: true };
    }
    case types.FETCH_CHAT_PENDING: {
      return { ...state, chat: true };
    }
    case types.CREATE_CHAT_PENDING: {
      return { ...state, createChat: true };
    }
    case types.JOIN_CHAT_PENDING: {
      return { ...state, joinChat: true };
    }
    case types.LEAVE_CHAT_PENDING: {
      return { ...state, leaveChat: true };
    }
    case types.DELETE_CHAT_PENDING: {
      return { ...state, deleteChat: true };
    }
    case types.SOCKETS_CONNECTION_PENDING: {
